...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  return {
    title: `${modelName} のスニーカー相場・最安値一覧 | Sneaker Checker`,
    description: `${modelName} の人気スニーカーの相場、定価、最安値推移をデータベースで一覧表示。お買い得なスニーカーを探せます。`,
  };
}
...
export default async function ModelPage({ params }: PageProps) {
  ...
  // スラッグからモデル名への変換 (例: air-jordan-1 -> air jordan 1)
  const modelQuery = modelSlug.replace(/-/g, ' ');
  ...
  const displayModel = sneakers[0].model || modelQuery;
    ...
    <div className={styles.main}>
      <div className={styles.heroSection} style={{ padding: '3rem 1rem 2rem' }}>
        <div className={styles.heroContent}>
          <h1 className={styles.heroTitle}>{displayBrand} {displayModel} 相場データベース</h1>
          <p className={styles.heroSubtitle}>
            {displayModel} のスニーカーの最安値と価格推移を記録しています。
          </p>
        </div>
      </div>
